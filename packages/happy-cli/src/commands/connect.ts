...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('happy connect')} - Connect AI vendor API keys to AgentsMaestro cloud

${chalk.bold('Usage:')}
  happy connect codex        Store your Codex API key in AgentsMaestro cloud
  happy connect claude       Store your Anthropic API key in AgentsMaestro cloud
  happy connect gemini       Store your Gemini API key in AgentsMaestro cloud
  happy connect status       Show connection status for all vendors
  happy connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in AgentsMaestro cloud. This enables you to use these services through AgentsMaestro
...
async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to AgentsMaestro cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with AgentsMaestro'));
